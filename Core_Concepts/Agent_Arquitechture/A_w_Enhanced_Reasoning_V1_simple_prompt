#NOTE - I deleted the system message, we cna update this one later if we need a more specific use case
#NOTE - We want the Model to improve its reasoning before answering, so lets do this first
#with a more simple System Prompt Engineering.

#NOTE - so this is as easy as updating the System_message... but will not 
#work so well with small Models, like Llama 8B   also not so good with 
#Deepseek as the model has embedded this <think> part
